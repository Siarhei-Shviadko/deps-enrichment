...
APP_NAME = api
NO_DEV_DOCKER_IMAGE = enrichment
DEV_DOCKER_IMAGE = enrichment-dev
...
build-prod:
	$(call build_service,enrichment-dev,./etc/enrichment/Dockerfile,,develop)
	$(call build_service,enrichment,./etc/enrichment/Dockerfile,,,enrichment-dev)
	$(call build_service,enrichment-migrator,./etc/migrator/Dockerfile)

.PHONY: push
## Push images to registry
push:
	$(call push_service,enrichment)
	$(call push_service,enrichment-dev)
	$(call push_service,enrichment-migrator)
...
tag:
	$(call tag_service,enrichment)
	$(call tag_service,enrichment-dev)
	$(call tag_service,enrichment-migrator)

.PHONY: pull
## Pull service images from docker registry
pull:
	$(call pull_service,enrichment)
	$(call pull_service,enrichment-dev)
	$(call pull_service,enrichment-migrator)
...
helm-upgrade-service:
	helm upgrade --install $(CI_PROJECT_NAME) .helm/services \
        ...
        --set enrichment.image.tag=$(commit_short_sha) \
        --set enrichment_consumer.image.tag=$(commit_short_sha) \
        --set enrichment_settings.DATABASE_REQUIRE_SECURE_TRANSPORT=$(DATABASE_REQUIRE_SECURE_TRANSPORT) \
        --set enrichment_settings.EXTERNAL_URL=$(SERVICE_EXTERNAL_URL) \
...
helm-migration-rollback:
	make migration_command="rollback $(commit_short_sha)" helm-apply-migrations
...
.PHONY: helm-upgrade
helm-upgrade:
	make migration_command="tag $(commit_short_sha)" helm-apply-migrations
...
.PHONY: build-no-dev
build-no-dev:
	$(call build_service,$(NO_DEV_DOCKER_IMAGE),./etc/enrichment/Dockerfile,,build,$(DEV_DOCKER_IMAGE))
