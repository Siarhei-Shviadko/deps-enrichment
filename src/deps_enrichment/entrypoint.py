...
from deps_enrichment import api, constants, messaging
from deps_enrichment.api.error_handlers import (
    json_enrichment_error_handler,
...
from deps_enrichment.settings import Settings
...
def register_auth(app: FastAPI):
    ...
    async def handle_authorization(request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        try:
            ...
            return json_enrichment_error_handler(err, err.status_code)
...
def run_api():
    ...
    uvicorn.run("deps_enrichment.entrypoint:create_fastapi", **options)
...
def run_message_dispatcher() -> None:
    ...
    containers.document_type_service().initialize()
