...
class DocumentTypeRepository(IDocumentTypeRepository):
    ...
    def save(self, document_type: DocumentType) -> None:
        ...
        with self._db.connection() as conn:
            conn.execute(self._query_factory.insert_document_type_with_update(), raw_document_type)
    ...
    def save_new_all(self, document_types: list[DocumentType]) -> None:
        ...
        with self._db.connection() as conn:
            conn.execute(self._query_factory.insert_document_types_without_update(raw_document_types))
